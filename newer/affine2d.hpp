...
namespace affine
{
    ...
    public:
        ...
        */
        ...
        {
            assert(tol > 0);
            return abs(m[i00]) < tol && abs(m[i01]) < tol && abs(m[i10]) < tol && abs(m[i11]) < tol && abs(d[0]) < tol && abs(d[1]) < tol;
        }
        ...
        bool almost_equal(const Affine2D &b, R tol = DEFAULT_TOLERANCE) const
        {
            ...
            */
            assert(tol > 0);
            return abs(m[i00] - b.m[i00]) < tol && abs(m[i01] - b.m[i01]) < tol && abs(m[i10] - b.m[i10]) < tol && abs(m[i11] - b.m[i11]) < tol && abs(d[0] - b.d[0]) < tol && abs(d[1] - b.d[1]) < tol;
        }
        ...
        {
            return "[" + std::to_string(m[i00]) + "," + std::to_string(m[i01]) + ";" + std::to_string(m[i10]) + "," + std::to_string(m[i11]) + "]+(" + std::to_string(d[0]) + "," + std::to_string(d[1]) + ")";
        }
        R det() const
        {
